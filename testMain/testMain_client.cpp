...
void main()
{
	...
	std::map<std::string, std::pair<std::string, USHORT>> serverList;
	serverList["echo"] = std::pair<std::string, USHORT>("127.0.0.1", 6000);


	NetworkClient c;
	if (!c.start(serverList))
		return;
